...
public interface CrudVoteRepository extends JpaRepository<Votes, Integer> {
    ...
    @Query("SELECT v FROM Votes v JOIN FETCH v.user WHERE v.user.id=:id AND v.date BETWEEN :beginDate AND :endDate ORDER BY v.date, v.restaurant.id")
    List<Votes> getByUserDate(@Param("id") int id, @Param("beginDate") LocalDate beginDate, @Param("endDate") LocalDate endDate);

    @Query("SELECT v FROM Votes v JOIN FETCH v.user WHERE v.user.id=:id ORDER BY v.date, v.restaurant.id")
    List<Votes> getByUser(@Param("id") int id);

    @Query("SELECT v FROM Votes v JOIN FETCH v.user WHERE v.date BETWEEN :beginDate AND :endDate  ORDER BY v.date, v.restaurant.id")
